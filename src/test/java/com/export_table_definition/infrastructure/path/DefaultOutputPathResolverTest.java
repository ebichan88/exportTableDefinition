...
public class DefaultOutputPathResolverTest {
    ...
    void testResolveTableDefinitionFile() {
        Path result = resolver.resolveTableDefinitionFile(baseInfo, table("public", "orders", "view"), baseDir);
    ...
    @Test
    @DisplayName("resolveErDiagramGroupFile(ページ): {base}/erDiagram_{DB名}_{スキーマ名}_group{グループ番号}_{ページ番号}.md")
    ...
    void testDifferentSchemasProduceDifferentDirectories() {
        Path publicDir = resolver.resolveTableDefinitionDirectory(baseInfo, table("public", "orders", "table"),
                baseDir);
        Path salesDir = resolver.resolveTableDefinitionDirectory(baseInfo, table("sales", "orders", "table"),
                baseDir);
