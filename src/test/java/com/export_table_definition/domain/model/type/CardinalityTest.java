...
public class CardinalityTest {
    ...
    void testNotationDiffersByRelationType() {
        for (Cardinality cardinality : Cardinality.values()) {
            assertNotEquals(cardinality.getNotation(RelationType.PHYSICAL),
                    cardinality.getNotation(RelationType.LOGICAL), cardinality.name());
        }
    }
    ...
    void testFromLabel() {
        for (Cardinality cardinality : Cardinality.values()) {
            assertEquals(cardinality, Cardinality.fromLabel(cardinality.getLabel()).orElseThrow(),
                    cardinality.name());
        }
        // 前後の空白は無視する
