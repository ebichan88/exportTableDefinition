...
public class ErDiagramTemplatesTest {
    ...
    private String erDiagram(List<ForeignKeyEntity> foreignKeys, int maxNodes) {
        return ErDiagramTemplates.erDiagram(foreignKeys, ErDiagramTemplates.diagramNodes(foreignKeys), maxNodes);
    }
    ...
    void testDiagramNodes() {
        ...
        var fk2 = newFk("public", "items", "fk_items_orders", "public", "orders");
        assertEquals(List.of(TableKey.of("master", "customers"), TableKey.of("public", "items"),
                TableKey.of("public", "orders")), ErDiagramTemplates.diagramNodes(List.of(fk1, fk2)));
    }
    ...
    void testDiagramTableLine() {
        ...
        assertTrue(line.startsWith("| 1 | public | orders | 受注 | table | [■](./TEST_DB/public/table/orders.md) |"));
    }

    @Test
    @DisplayName("diagramTableLine: 出力対象範囲外のテーブルはリンクを張らない")
    void testDiagramTableLineWithoutDefinition() {
        String line = ErDiagramTemplates.diagramTableLine(1, TableKey.of("external", "master_data"), null);
