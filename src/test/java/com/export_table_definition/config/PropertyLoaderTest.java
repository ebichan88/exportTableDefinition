 ...
 * PropertyLoaderはプロパティファイルの探索先ディレクトリ（{@code conf} または
 * {@code src/main/resources/conf}）をハードコードで解決する設計のため、
 * 専用のテスト用プロパティファイル（{@code src/main/resources/conf/PropertyLoaderTestFixture.properties}）を
 * 用意し、実際のファイルシステム経由での読み込みを検証する
 */
public class PropertyLoaderTest {
    ...
    void testGetStringMissingKeyThrows() {
        assertThrows(MissingResourceException.class, () -> PropertyLoader.getString(FIXTURE, "doesNotExist"));
    }

    @Test
    @DisplayName("getList: カンマ区切りの値を分割し、空要素のみ除去する（前後の空白はトリムしない）")
    void testGetListSplitsCommaSeparatedAndRemovesBlankElementsOnly() {
        assertEquals(List.of("alpha", "beta", " gamma ", "delta"), PropertyLoader.getList(FIXTURE, "csvValue"));
    }
