...
public interface ExportTableDefinitionUsecase {
    ...
    /**
     * DBの現状から生成したドキュメントと、{@code outputPath}配下に既にコミット済みのドキュメントを比較し、
     * 差分（＝ドキュメントの再生成・コミット忘れ）を検知するメソッド<br>
     * 内部で一時ディレクトリへ向けて{@link #exportTableDefinition}相当の処理を実行し、
     * 生成結果と{@code outputPath}配下の既存ファイルをファイル単位で比較する
     *
     * @param targetSchemaList テーブル定義出力対象のスキーマのリスト
     * @param targetTableList  テーブル定義出力対象のテーブルのリスト
     ...
     * @return 比較結果
     */
    public DiffResult checkDocumentDiff(List<String> targetSchemaList, List<String> targetTableList, String outputPath,
            int chunkSize, int erDiagramMaxNodes, List<String> outputObjectList, String annotationPath);
}
