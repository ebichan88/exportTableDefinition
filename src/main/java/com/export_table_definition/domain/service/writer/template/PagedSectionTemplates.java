 ...
 * GitHub上で表として表示できる行数には上限があるため、一覧系のドキュメントは行数が多い場合に
 * 別ファイルへ分割して出力する。本体ページに載せるリンク一覧と、分割ページのページ送りを扱う
 *
 * @since 1.0
 * @version 1.0
...
public class PagedSectionTemplates {
    ...
    public static String pagedSectionLinks(String heading, String label, List<String> pageHrefs) {
        ...
        for (int i = 0; i < pageHrefs.size(); i++) {
            sb.append(String.format("* [%s_%d](%s)  ", label, i + 1, pageHrefs.get(i))).append(LINE_SEPARATOR);
        }
     ...
     * @return フッター文字列
     */
    public static String pageFooter(String prevHref, String nextHref, String backHref, String backLabel) {
        ...
        return sb.append(String.format("[%s](%s)", backLabel, backHref)).append(LINE_SEPARATOR).toString();
    }
}
