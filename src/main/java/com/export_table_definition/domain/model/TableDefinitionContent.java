...
public record TableDefinitionContent(BaseInfoEntity baseInfo, TableEntity table, List<ColumnEntity> columns,
        ...
        List<ForeignKeyEntity> logicalRelations, List<ForeignKeyEntity> incomingForeignKeys,
        List<TriggerEntity> triggers, TableAnnotation annotation, Path outputBaseDir) {

    /**
     * テーブル定義出力に必要な情報をまとめたレコードを組み立てる<br>
     * 参照側（自テーブル → 参照先）の関連は、DBに実在する外部キー制約（{@code foreignKeys}）と
     * サイドカーYAML由来の論理リレーション（{@code logicalRelations}）に分けて保持する。
     * テーブル定義書では別々のセクションへ掲載し、ER図では両者を1つの図にまとめて描画するため。
     * 被参照側（{@code incomingForeignKeys}）はER図でしか用いないため由来を分けない
     *
     * @param baseInfo
     * @param table
     * @param columns
    ...
    public static TableDefinitionContent assemble(BaseInfoEntity baseInfo, TableEntity table, Columns columns,
        ...
        return new TableDefinitionContent(baseInfo, table, columns.of(table), indexes.of(table), constraints.of(table),
                foreignkeys.physicalOf(table), foreignkeys.logicalOf(table), foreignkeys.incomingOf(table),
                triggers.of(table), annotations.of(table), baseDir);
    }
