 ...
 * DBに実在する外部キー制約（{@link RelationType#PHYSICAL}）と、サイドカーYAMLで宣言された
 * 論理リレーション（{@link RelationType#LOGICAL}）の双方を表す。
 * 両者はER図では同じグラフ上に描画されるが、掲載セクションと線種によって区別される
 *
...
public record ForeignKeyEntity(String schemaName, String tableName, String foreignkeyInfo, String foreignkeyName,
        ...
        Cardinality cardinality, RelationType relationType) implements SchemaTableKeyed {
    ...
    public ForeignKeyEntity(String schemaName, String tableName, String foreignkeyInfo, String foreignkeyName,
            String columnNames, String referenceSchemaName, String referenceTableName, String referenceColumnNames) {
        this(schemaName, tableName, foreignkeyInfo, foreignkeyName, columnNames, referenceSchemaName,
                referenceTableName, referenceColumnNames, Cardinality.ONE_TO_MANY, RelationType.PHYSICAL);
    }

    /**
     * サイドカーYAML由来の論理リレーションを生成する静的ファクトリメソッド<br>
     * DBに外部キー制約が存在しないため、多重度は機械的に判定できない。
     * 呼び出し側でYAMLの明示指定を解決した上で渡すこと
     *
     * @param schemaName           参照元（子）スキーマ名
     * @param tableName            参照元（子）テーブル名
     ...
     */
    public static ForeignKeyEntity logical(String schemaName, String tableName, String relationName,
            String columnNames, String referenceSchemaName, String referenceTableName, String referenceColumnNames,
            Cardinality cardinality) {
        return new ForeignKeyEntity(schemaName, tableName, "", relationName, columnNames, referenceSchemaName,
                referenceTableName, referenceColumnNames, cardinality, RelationType.LOGICAL);
    }
