...
annotationPath={手動付帯情報を記述したサイドカーYAMLのパス} ※空白の場合はマージを行わない
```
...
`annotationPath`は、DBからは取得できない手動の付帯情報（テーブル説明・テーブル備考・カラム備考）を、テーブル定義書の再生成時にマージするためのサイドカーファイルのパスです。テーブル定義書は毎回全上書きで生成されるため、生成物のMarkdownに直接書き込んだ付帯情報は次回の生成で失われます。付帯情報をこのサイドカーYAMLに外出ししておくことで、DBの変更に追従して定義書を再生成しても手書き情報が保持されます。空白（未指定）の場合はマージを行わず、従来どおり付帯情報は空欄で出力されます。

サイドカーYAMLは「スキーマ.テーブル」をキーとした以下の構造で記述します（`sample/annotations.sample.yml`も参照）。
