...
| `1` | `--check`モードで差分あり |
| `2` | 失敗（設定の誤り、DBに接続できない、出力先に書き込めない等） |
...
`conf`ディレクトリ・設定ファイル・必須のキーが見つからない場合や、`outputObjects`に未知の種別名を指定した場合など、設定に誤りがある場合はDBへ接続する前に`[result]:FAIL`（終了コード`2`）で終了します。
...
* 差分が1件でも見つかった場合は終了コード`1`、差分がない場合は`0`、設定の誤りやDBに接続できない等で比較処理自体が失敗した場合は`2`で終了します。CIのジョブをそのまま失敗させられるほか、「差分あり」と「比較自体の失敗」を終了コードで区別できます。
